<template>
    <div>
        <a class='mr-2' v-for="(count, i) in countResults" :key="i" v-on:click.prevent="buscarcat(count-1)"> PAGINA {{count}}</a>
        <br>
        <br>

            <div class="relative mx-auto text-gray-600"  style="float:left">  
                <input class="border-8 border-gray-300 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none" ref='search' id='search' type="search" placeholder="Busca" @keyup="buscarcat()">
            </div>
            <br>
            <br>
